...
export async function updateUserService(user: IUserUpdate, id: string): Promise<User> {
    const userRepository = AppDataSource.getRepository(User)

    const userKeys = Object.keys(user)

    if(userKeys.includes('isActive')){
        throw new AppError(401, "Is not able to update isActive value")
    }

    if(userKeys.includes('id')){
        throw new AppError(401, "Is not able to update id")
    }
